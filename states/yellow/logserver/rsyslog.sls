...
logserver.{{params.name}}.check.config:
  cmd.run:
    - name: docker run --rm -v {{params.host.config}}/rsyslog.d:/etc/rsyslog.d -v {{params.host.config}}/lognorm:/etc/lognorm -e "LIBLOGNORM_RULEBASES=/etc/lognorm" --name {{params.name}}-test markuskont/rsyslog:latest -N 1
    - require:
      - pkg: docker
